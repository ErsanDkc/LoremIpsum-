...
formDOM.addEventListener("submit", function(e) {
    e.preventDefault();
    let amount = parseInt(paragraf.value)
    let random = Math.floor(Math.random()* text.length)
    
    if(isNaN(amount) || amount<0 || amount> text.length) {
        articleDOM.innerHTML = `<p class="p">${text[random]}</p>`
    }
    else {
        let reqText = text.slice(0,amount);

        reqText = reqText.map((item) => {
         return   `<p class="p">${item}</p>`
        }).join("")
        
        articleDOM.innerHTML = reqText
